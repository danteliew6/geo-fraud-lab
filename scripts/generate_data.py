#!/usr/bin/env python3
"""
geo-fraud-lab — deterministic synthetic data generator for a generic
digital-lending / consumer-loan **geospatial fraud** analytics lab.
...
GEOSPATIAL signals only:
  - location_mismatch : application far from the borrower's home province
  - impossible_travel : two apps by one customer, far apart, short time gap
  - device_ring       : one device_id reused across many customers
  - foreign_ip        : ip_country != 'ID'
  - geo_hotspot       : fraud concentrated in a few provinces / H3 cells

Currency is Indonesian Rupiah (IDR); the geography (provinces, lat/lon) drives
the geospatial angle. H3 cells (h3_res7, h3_res9) are left NULL here and
computed in SQL via h3_longlatash3string(lon, lat, res) on a Photon/serverless
SQL warehouse after load.
"""
...
# Fixed seed → identical dataset for every user of the lab.
SEED = 42

# Volume targets (deterministic given SEED).
N_CUST = 20_000
N_APP = 30_000
N_LOAN = 18_000
...
# ---------------------------------------------------------------------------
# Indonesian geography: province -> (representative city, lat, lon, weight)
# Weights approximate population share so volume concentrates in Java.
# Province names are spelled to match standard geocoding for filled/region maps.
# ---------------------------------------------------------------------------
PROVINCES = [
...
# Neutral, non-branded product name.
PRODUCT_NAME = "Personal Cash Loan"

# Generic Indonesian name pools (no real individuals).
...
def province_frame() -> pd.DataFrame:
    """Province centroid reference (also written as dim_province)."""
    return pd.DataFrame(
        [(p[0], p[1], p[2], p[3]) for p in PROVINCES],
        columns=["province", "capital", "center_lat", "center_lon"],
    )


def generate() -> dict[str, pd.DataFrame]:
    """Build every table as a pandas DataFrame. Deterministic given SEED."""
    ...
    # -----------------------------------------------------------------------
    # dim_product : one row per tenor of a generic personal cash loan
    # -----------------------------------------------------------------------
    tenors = [6, 9, 12, 15, 18, 24]
    rates = [0.36, 0.40, 0.42, 0.44, 0.46, 0.48]  # p.a. (typical unsecured microloan band)
    ...
    # -----------------------------------------------------------------------
    # fact_loan_application (30k) with geo + fraud
    # -----------------------------------------------------------------------
    app_cust_idx = rng.integers(0, N_CUST, N_APP)  # customers can have multiple apps
    ...
    # Default: application near home (jitter ~ up to ~10km)
    ...
    # --- device_ring: ~15 devices reused across many apps (~1.2% of apps)
    N_RING = 15
    ...
    # --- location_mismatch: app placed in a DIFFERENT far province (~1.0%)
    ...
    # --- foreign_ip: ip_country outside ID (~0.5%)
    ...
    # --- geo_hotspot: concentrate extra fraud in two hotspot provinces (~0.5%)
    ...
    # --- impossible_travel: same-customer consecutive apps far apart in short time.
    avail = np.setdiff1d(np.arange(N_APP), np.flatnonzero(is_fraud))
    n_pairs = 120
    pair_pool = rng.choice(avail, size=n_pairs * 2, replace=False)
    it_first = pair_pool[:n_pairs]   # legit anchor near home
    it_second = pair_pool[n_pairs:]  # fraudulent far-away app shortly after
    for a, b in zip(it_first, it_second):
        c = app_cust_idx[a]
        app_cust_idx[b] = c  # force same customer as the anchor
        ...
        gap_sec = int(rng.integers(1800, 3 * 3600))  # 0.5-3h later
    ...
    # decision: approved/rejected. Fraud and poor credit lower approval.
    cust_credit = credit_band[app_cust_idx]
    approve_p = np.select(
        [cust_credit == "Excellent", cust_credit == "Good", cust_credit == "Fair", cust_credit == "Poor"],
        [0.90, 0.78, 0.60, 0.35], default=0.65)
    approve_p = np.where(is_fraud, approve_p * 0.5, approve_p)  # fraud caught sometimes
    ...
    # -----------------------------------------------------------------------
    # fact_loan (funded from approved applications)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # fact_repayment (~200k): installment schedule elapsed to date
    # -----------------------------------------------------------------------
    ...
    # NOTE: dim_province is created authoritatively in sql/02_dim_province.sql
    # (province centroids live in one place), so it is not written here.
    return {
...
def summarize(tables: dict[str, pd.DataFrame]) -> None:
    ...
    app = tables["fact_loan_application"]
    print(f"Fraud apps: {int(app['is_fraud'].sum()):,} ({app['is_fraud'].mean() * 100:.2f}%)")
